...
<?php
	...
	if ($result->rowCount() == 0)
	{
		...
		{
			echo("<tr>");
			echo("<td><a href=\"display_consults.php?\"> {$row['animal']} </a></td>");
			echo("<td>{$row['vat']}</td>");
		...
		# Check if the client is registered in the database (Not working properly yet!!!!!)
		$sql = "SELECT * FROM client WHERE VAT = :vat;";
		$st = $connection->prepare($sql,array(':vat'=>$clientVAT));
		if ($st == FALSE){
			$info = $connection->errorInfo();
			echo("<p>Error: {$info[2]}</p>");
			exit();
		}else{
			#sth->bind_param(':vat', $clientVAT);
			$st->execute();
			$res = $st->fetchAll();
			if ($res->rowCount() == 0)
			{	
				...
				echo("<h4>This client is not registered: <form action='NewClient.php' method='post'>\n<p><input type='submit' value='NewClient'/></p>\n</form></h4>");
			}
		}
